...
def make_base_rates(laser_factors):
    ...
    r = {}     # dictionary of rates (in TPG seconds) generated by (910,000/f)
    for i in iters:
        for c in i:
            q = np.prod(np.array(c))
            f.add(q)
    for factor in f:
        rate = 910000//28//factor
        r[factor] = rate
    return r


def allowed_goose_rates(base_rate, rate_dict):
    """
    ...
    """
    d = {}
    base_div = rate_dict[base_rate]
    for rate in rate_dict.keys():
        if rate < base_rate:
            if rate_dict[rate] % base_div == 0:
                d[rate] = rate_dict[rate]
    # Add the zero goose case
    d[0] = None
